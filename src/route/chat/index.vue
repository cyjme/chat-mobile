      ...
      <div id="chat-messages" v-on:scroll="handleScroll">
        ...
        <div v-for="msg in msgList" :class="msg.to==to?'message right':'message'" :key="msg.msgId">
          <template v-if="msg.to == to">
            ...
            <img v-if="currentUser.avt==''?false:true"
                 :src="'https://qiniu.ideapar.com/'+JSON.parse(currentUser.avt)[0]+'?imageMogr2/thumbnail/80x/strip/quality/50/format/jpg'"/>
          </template>
          <template v-if="msg.to != to">
            <div v-if="profile.avt==''?true:false" class="text-avatar">{{profile.nick.substring(0,1)}}</div>
            <img v-if="profile.avt==''?false:true"
                 :src="'https://qiniu.ideapar.com/'+JSON.parse(profile.avt)[0]+'?imageMogr2/thumbnail/80x/strip/quality/50/format/jpg'"/>
          </template>
          ...
          <div class="bubble">
            ...
            <span>{{(new Date() - new Date(msg.created_at))/(24*60*60*1000) > 1? new Date(msg.created_at).getMonth()+1 +'月'+new Date(msg.created_at).getDate() +"日":''}}
              {{new Date(msg.created_at).getHours()+":"+(new Date(msg.created_at).getMinutes()<10 ? "0"+new Date(msg.created_at).getMinutes(): new Date(msg.created_at).getMinutes())}}</span>
          </div>
        </div>
      </div>
